      ...
      <div className="container max-w-[960px] mx-auto px-6">
        ...
        <ScrollReveal delay={0.1}>
          ...
          <div className="flex items-center justify-center gap-3 mb-14">
            <span className={`text-[13px] cursor-pointer transition-colors ${!yearly ? "text-foreground font-medium" : "text-muted-foreground"}`} onClick={() => setYearly(false)}>Mensal</span>
            <button onClick={() => setYearly(!yearly)} className={`relative w-10 h-[22px] rounded-full transition-colors duration-200 ${yearly ? "bg-primary" : "bg-muted"}`}>
              <span className={`absolute top-[3px] left-[3px] w-4 h-4 rounded-full bg-foreground transition-transform duration-200 ${yearly ? "translate-x-[18px]" : "translate-x-0"}`} />
            </button>
            <span className={`text-[13px] cursor-pointer transition-colors ${yearly ? "text-foreground font-medium" : "text-muted-foreground"}`} onClick={() => setYearly(true)}>
              Anual<span className="ml-1.5 text-[11px] text-primary font-medium">-33%</span>
            </span>
          </div>
        </ScrollReveal>
        ...
        <ScrollReveal delay={0.2}>
          <div className="grid md:grid-cols-3 gap-px rounded-xl border border-border/50 overflow-hidden bg-border/50">
            {plans.map((plan, i) => (
              <div key={i} className={`relative flex flex-col p-7 ${plan.highlighted ? "bg-primary/[0.03]" : "bg-background"}`}>
