...
const DICTATION_TEXT =
  "Fígado de dimensões ligeiramente aumentadas, bordas rombas, ecotextura sólida com aumento difuso da ecogenicidade e atenuação posterior. Vesícula biliar normodistendida, paredes finas, sem cálculos. Hepatocolédoco de calibre preservado...";
...
export default function HeroDemo() {
  ...
  // Recording phase — type dictation text
  useEffect(() => {
    if (phase !== "recording") return;
    const id = setInterval(() => {
      setTypedChars((c) => {
        if (c >= DICTATION_TEXT.length) {
          clearInterval(id);
          // Move to processing after a beat
          setTimeout(() => {
            if (isHovered.current) setPhase("processing");
          }, 600);
          return c;
        }
        return c + 2;
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      setProcessProgress((p) => {
        if (p >= 100) {
          clearInterval(id);
          setTimeout(() => {
            if (isHovered.current) setPhase("result");
          }, 400);
          return 100;
        }
        return p + 2;
    ...
    // Steps appear at 0%, 33%, 66%
    const s1 = setTimeout(() => setProcessStep(1), 0);
    const s2 = setTimeout(() => setProcessStep(2), 500);
    const s3 = setTimeout(() => setProcessStep(3), 1000);
    return () => { clearInterval(id); clearTimeout(s1); clearTimeout(s2); clearTimeout(s3); };
  }, [phase]);

  // Result phase — reveal sections one by one
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      if (i >= LAUDO_SECTIONS.length) {
        clearInterval(id);
        // After showing all, wait then loop
  ...
  const handleMouseLeave = () => {
    ...
    // Let current phase finish, then pause
  };
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-bold text-destructive tabular-nums ml-1">
                    {fmtTime(timer)}
                  </span>
                </div>
          ...
          <div className="flex flex-col items-center justify-center h-[300px] gap-5 animate-fade-in">
            ...
            <div className="text-center">
              ...
              <div className="text-[12px] text-muted-foreground">
                Identificando achados e estruturando laudo
              </div>
            </div>
              ...
              {LAUDO_SECTIONS.map((section, i) => (
                <div
                  ...
                  className={`transition-all duration-500 ${
                    ...
                    section.isImpression
                      ? "bg-primary/5 border border-primary/15 rounded-lg p-3 mt-3"
                      : "bg-background/40 border border-border/30 rounded-lg p-3"
                  }`}
                >
                  <div
